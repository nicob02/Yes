...
class SolveGradientsLST(object):
    ...
    def solve_single_varible(self, graph:Data, u):
        ...
        pos = graph.pos # Nodes position
        edges = graph.edge_index
        
        u_differ = u[edges[0]] - u[edges[1]]    # Edge[0] = starting node, other is ending node
        ...
        if len(self.w_dict)==0:
            ...
            for node_index in range(pos.shape[0]):
                u_edge_index = torch.where(edges[1]==node_index)[0] # That end at node i, aka node i's neighbours
                u_ = u_differ[u_edge_index]
                number_u_ = u_.shape[0]
                A = dydx[edges[1]==node_index].detach()
                W = torch.inverse(A.t().mm(A)).mm(A.t())    # Weight matrix
                self.w_dict[number_u_].append(W)        # W matrix stored by its key which is #neighbours 
                self.index_dict[number_u_].append(u_edge_index)
                self.node_index_dict[number_u_].append(node_index)  
                dudxdy[node_index] = W.mm(u_).squeeze() # Gradient matrix which is u diff multiplied by weight matrix.
                
            
        else:
            
            for number_u_ in self.w_dict.keys():    #Loops through 
                
                W = torch.stack(self.w_dict[number_u_])
                U = u_differ[torch.cat(
                    self.index_dict[number_u_], dim=-1)].reshape((W.shape[0], -1, 1))
