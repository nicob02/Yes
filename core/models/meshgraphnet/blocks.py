...
class EdgeBlock(nn.Module):
    ...
    def forward(self, graph):
        ...
        v = self.sender_func(senders_attr) //node attribute
        v = self.receiver_func(receivers_attr) + v //node receiver attribute
        v = self.edge_func(edge_attr) + v       // concatenation of old edge attribute + new info
...
class NodeBlock(nn.Module):
    ...
    def forward(self, graph):
        ...
        nodes_to_collect.append(graph.x)        // Previous node attributes
        nodes_to_collect.append(agg_received_edges) // new node attributes info, summation of neighboring edge attributes
        collected_nodes = torch.cat(nodes_to_collect, dim=-1)
        x = self.net(collected_nodes) // new node attributes, encoded/transformed by MLP
        graph.x = x
        return graph
